...
function filtrar(bandas){
    ...
    nombre.includes(buscador.value))
    renderizarTarjetas(arrayFiltrado)
}
    
function renderizarTarjetas(arrayDeBandas){
    let contenedorProductos= document.getElementById("wrapper")
    ...
    // let botones= document.getElementsByClassName("agregarAlCarrito")
...
function renderizarCarrito(arrayDeBandas){
    ...
    let carritoDOM= document.getElementById("contenedorCarrito")
    carritoDOM.innerHTML= ""
    arrayDeBandas.forEach(({ nombre, img, precio, unidades, subtotal 
        ...
        carritoDOM.innerHTML += 
         ...
         carritoDOM.innerHTML += `<div class="col"><button class="btn btn-primary mg-5" type="button" id="comprar">Finalizar compra</button></div></div>`
...
function mostrarCarrito(){
    let contenedorProductos= document.getElementById("carritoDOM")
